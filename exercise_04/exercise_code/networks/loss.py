...
class BCE(Loss):
    ...
    def forward(self, y_out, y_truth):
        ...
        result = -np.array([y_truth[i] * np.log(y_out[i]) + (1 - y_truth[i]) * np.log(1 - y_out[i]) for i in range(0, n)])
    ...
    def backward(self, y_out, y_truth):
        ...
        result = np.array([-y_truth[i]/y_out[i] + (1 - y_truth[i])/(1 - y_out[i]) for i in range(0, len(y_truth))])
